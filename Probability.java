...
public class Probability {
	...
	public void dpTableFullBudget() {
		
		System.out.println("Iterated version with full budget:");
		...
		for(int j = 0; j < column; j++){
            int k = (j/cost[0]);
            T[0][j] = (1 - Math.pow((1 - reliability[0]), k));  
            V[0][j] = k;
        }
            ...
            for(int i = 1; i < machine; i++) {
                ...
                for(int k = 0; k*cost[i] <= j; k+=1){ 
                    double value = (T[i-1][j-(k*cost[i])])*
                        (1.0-Math.pow((1.0-reliability[i]),k));
		...
		for (int i = row -1; i >= 0; --i) {
			System.out.print(V[i][remainingBudget]);
